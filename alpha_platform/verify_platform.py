...
from alpha_platform.market_data.news_filter import NewsFilter, NewsEvent, NewsImpact
...
def run_system_verification():
    ...
    # 2. Test Real-Time Economic News Filter
    news_filter = NewsFilter()
    is_blocked, _ = news_filter.is_news_blocked("EURUSD")
    print(f"[2/9] Real-Time Economic News Filter (Blocked={is_blocked}): PASSED")
    ...
    print(f"[3/9] Centralized Feature Store: PASSED ({len(feats)} features computed)")

    # 4. Test Strategy Candidates (Trend, Breakout, Mean Reversion)
    ...
    print(f"[4/9] Alpha Strategy Generators: PASSED ({len(candidates)} trade candidates generated)")

    # 5. Test Meta-Labeling & Probability Calibration
    ...
    print(f"[5/9] AI Meta-Labeling & Calibration (Brier: {train_res['brier_score']:.4f}): PASSED")

    # 6. Test Statistical Validation Gate (PBO & DSR)
    ...
    print(f"[6/9] Statistical Validation Gate (PBO={val_res['pbo']:.2f}, DSR={val_res['dsr']:.2f}): PASSED")

    # 7. Test HRP Portfolio Allocator
    ...
    print(f"[7/9] Hierarchical Risk Parity (HRP) Allocator: PASSED ({weights})")

    # 8. Test Risk Engine Authority
    ...
    print(f"[8/9] Risk Engine Authority Check: PASSED (Position Scaled Size: {verdict.scaled_position_size:.2f})")

    # 9. Test Stress Testing Engine
    ...
    print("[9/9] Stress Testing Engine: PASSED")
